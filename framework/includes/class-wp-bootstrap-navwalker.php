	...
	class WP_Bootstrap_Navwalker extends Walker_Nav_Menu {
		...
		public function start_el( &$output, $item, $depth = 0, $args = null, $id = 0 ) {
			...
			if ( false !== strpos( $args->items_wrap, 'itemscope' ) && false === $this->has_schema ) {
				...
				if ( property_exists( $args, 'walker' ) && property_exists( $args->walker, 'has_children' ) && $args->walker->has_children && 0 === $depth ){
					$args->link_after .= '<i class="far fa-angle-down" aria-hidden="true"></i>';
				}
			}
			...
			elseif( $this->has_children && $depth > 0 ){
			...
			if ( is_single() && property_exists( $item, 'object' ) && property_exists( $item, 'menu_item_parent' ) && $item->menu_item_parent < 1 && get_post_type() === $item->object ) {
				$classes[] = ' active';
			}
			...
			if ( $this->has_children && 0 === $depth ) {
				if ( property_exists( $args, 'menu' ) && $args->menu === 'mobile-slide-menu' ) {
					$atts['href'] = '#';
			...
			$output .= apply_filters( 'walker_nav_menu_start_el', $item_output, $item, $depth, $this );
